...
#Reglas
#Regla 1: Si el servicio es malo Y la comida es rancia, entonces la propina es nivel trabajador
regla_1 = np.fmin(np.fmin(s_mal, c_ran), p_trabajador)
#Regla 2: Si el servicio es normal O la comida es normal, entonces la propina es nivel normal
regla_2 = np.fmin(np.fmax(s_nor, c_nor), p_normal)
...
plt.axvline(x=propina_centroide, label=f'Propina por centroide: {propina_centroide:.5f}%', color='b', linestyle='--')
plt.legend(loc='upper left')
plt.ylim([0, 1])
